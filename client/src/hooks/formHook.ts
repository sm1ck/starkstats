        ...
        fetch('/api/batchcheck', requestOptions)
            ...
            .then(data => {
                ...
                if (data.data !== undefined) {
                    setResultCsv([["#", "Адрес", "Баланс", "Транзакции", "Активных дней / недель / месяцев", "Последняя транзакция"], ...data.data.map((v: any, index: number) => [String(index + 1), String(v.contract), String(v.balance), String(v.nonce), String(v.txTimestamps), String(v.lastTx)])]);
                }
