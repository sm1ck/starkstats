...
try {
  ...
  if (isCache) {
    ...
    if (parentPort) {
      parentPort.postMessage({
        totalWallets: { data: { totalWallets, totalWalletsFiltered } },
      });
    }
    exit(0);
  }
  if (parentPort) {
    ...
    for await (let contract of Contract.find(database.filterOutdated)
      .limit(limit)
      .skip(skip)
      .lean()) {
      // volume
      if (contract.bridgesVolume < 0.01 && contract.bridgesVolume > 0) {
        ...
        typeof contract.bridgesVolume === "number" &&
        contract.bridgesVolume > 0
      ) {
        lessThan1++;
      }
      if (
        contract.bridgesWithCexVolume < 0.01 &&
        contract.bridgesWithCexVolume > 0
      ) {
        lessThan5of1000_2++;
      } else if (
        contract.bridgesWithCexVolume < 0.05 &&
        contract.bridgesWithCexVolume > 0
      ) {
        lessThan1of100_2++;
      } else if (
        contract.bridgesWithCexVolume < 0.1 &&
        contract.bridgesWithCexVolume > 0
      ) {
        lessThan1of10_2++;
      } else if (
        contract.bridgesWithCexVolume < 0.5 &&
        contract.bridgesWithCexVolume > 0
      ) {
        lessThan1of2_2++;
      } else if (
        typeof contract.bridgesWithCexVolume === "number" &&
        contract.bridgesWithCexVolume > 0
      ) {
        lessThan1_2++;
      }
      // internal volume
      let totalInternalVolume =
        ethPrice * contract.internalVolume + contract.internalVolumeStables;
      if (totalInternalVolume < 50 && totalInternalVolume > 0) {
        ...
        typeof totalInternalVolume === "number" &&
      ...
      if (timestamps.length > 0) {
        ...
        if (timestamps.length > 1) {
          let lastTimestampDays = timestamps[0] * 1000,
            lastTimestampWeeks = timestamps[0] * 1000,
            lastTimestampMonths = timestamps[0] * 1000;
          let skip = true;
          for (let timestamp of timestamps) {
            ...
            timestamp *= 1000;
            let lastDateDays = DateTime.fromMillis(lastTimestampDays, {
              zone: "utc",
            });
            let lastDateWeeks = DateTime.fromMillis(lastTimestampWeeks, {
              zone: "utc",
            });
            let lastDateMonths = DateTime.fromMillis(lastTimestampMonths, {
              zone: "utc",
            });
            ...
            if (
              current != last ||
              compareByYear(lastDateMonths.year, currentDate.year)
            ) {
    ...
    parentPort.postMessage({
      activity: {
        data: {
          users_by_days: uniqDays,
          users_by_weeks: uniqWeeks,
          users_by_months: uniqMonths,
        },
      },
    });
  }
